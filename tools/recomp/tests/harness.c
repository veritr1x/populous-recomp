...
void recomp_setjmp(X86 *c) { c->r[R_ESP] += 4; c->r[R_EAX] = 0; }
void recomp_longjmp(X86 *c) { (void)c; fprintf(stderr, "harness: guest longjmp\n"); abort(); }
uint32_t recomp_in(X86 *c, uint32_t p, int s) { (void)c; (void)p; (void)s; return 0; }
void recomp_out(X86 *c, uint32_t p, uint32_t v, int s) { (void)c; (void)p; (void)v; (void)s; }
void recomp_cli(X86 *c) { (void)c; }
void recomp_sti(X86 *c) { (void)c; }
void recomp_hlt(X86 *c) { (void)c; }
void recomp_int(X86 *c, uint32_t v) { (void)c; (void)v; }

/* --- driver interface ---------------------------------------------------- */

uint64_t harness_x86_size(void) { return (uint64_t)sizeof(X86); }
...
uint8_t *harness_mem(void)
{
    if (!g_mem) {
        ...
        if (!g_mem) { fprintf(stderr, "harness: out of memory\n"); abort(); }
    }
    return g_mem;
}
...
uint32_t harness_last_unknown(void) { return last_unknown; }
uint32_t harness_last_div_error(void) { return last_div_error; }

/* Run one translated function.  The caller has already put the guest stack in
 * c->r[R_ESP], with the return address at [ESP]. */
void harness_run(uint32_t addr, X86 *c) { recomp_call(c, addr); }
...
void harness_cpuid(uint32_t leaf, uint32_t out[4])
{
    ...
    c.r[R_EAX] = leaf;
    recomp_cpuid(&c);
    out[0] = c.r[R_EAX]; out[1] = c.r[R_EBX];
    out[2] = c.r[R_ECX]; out[3] = c.r[R_EDX];
}

uint32_t harness_eflags(const X86 *c) { return x86_get_eflags(c); }
...
static int harness_ftag_matches_libm(uint64_t bits)
{
    ...
    unsigned expected = cls == FP_ZERO ? FTAG_ZERO :
        (cls == FP_NAN || cls == FP_INFINITE) ? FTAG_SPECIAL : FTAG_VALID;
...
static int harness_ftag_sweep(void)
{
    ...
    const uint64_t mantissas[] = {0, 1, UINT64_C(0x7ffffffffffff),
        UINT64_C(0x8000000000000), UINT64_C(0x8000000000001),
        UINT64_C(0xffffffffffffe), UINT64_C(0xfffffffffffff)};
    ...
    for (unsigned i = 0; i < 100000; ++i) {
        ...
        if (!harness_ftag_matches_libm(bits)) return 0;
    }
    return 1;
}
...
#define CHECK(cond) do {                                                     \
    ...
    CHECK(fround_cw(&c, -2.5) == -2.0);
    c.fpu_cw = 0x077f; CHECK(fround_cw(&c, 2.7) == 2.0);   /* RC=01 down */
    c.fpu_cw = 0x0b7f; CHECK(fround_cw(&c, 2.7) == 3.0);   /* RC=10 up */
    c.fpu_cw = 0x0f7f; CHECK(fround_cw(&c, 2.7) == 2.0);   /* RC=11 truncate */
    c.fpu_cw = 0x037f;
    ...
    c.r[R_EAX] = 0; c.r[R_EDX] = 0x8000; idiv16(&c, 0xffff, 0);
    CHECK(last_div_error_count == before + 1);
    before = last_div_error_count;
    c.r[R_EAX] = 0; c.r[R_EDX] = 0x80000000u; idiv32(&c, 0xffffffffu, 0);
    CHECK(last_div_error_count == before + 1);
    /* x87 empty tracking and tag classification */
    c.fpu_tag = 0xffff; c.fpu_top = 0;
    CHECK(fempty(&c, 0));
    fpush(&c, 1.0);
    ...
    fpush(&c, 0.0);   CHECK(ftag_of(&c, c.fpu_top) == FTAG_ZERO);    fdrop(&c);
    fpush(&c, (double)INFINITY);
                      CHECK(ftag_of(&c, c.fpu_top) == FTAG_SPECIAL); fdrop(&c);
    fpush(&c, 5e-324);   /* smallest subnormal double: a NORMAL extended */
                      CHECK(ftag_of(&c, c.fpu_top) == FTAG_VALID);   fdrop(&c);

    /* FXAM classification, including the classes the old code got wrong */
    c.fpu_tag = 0xffff; c.fpu_top = 0; c.fpu_sw = 0;
    fxam(&c);         CHECK((c.fpu_sw & 0x4500) == 0x4100);   /* empty  */
    fpush(&c, 1.0);      fxam(&c); CHECK((c.fpu_sw & 0x4500) == 0x0400); fdrop(&c);
    fpush(&c, 0.0);      fxam(&c); CHECK((c.fpu_sw & 0x4500) == 0x4000); fdrop(&c);
    fpush(&c, (double)INFINITY);
                         fxam(&c); CHECK((c.fpu_sw & 0x4500) == 0x0500); fdrop(&c);
    ...
    fpush(&c, 5e-324);   fxam(&c); CHECK((c.fpu_sw & 0x4500) == 0x0400); fdrop(&c);
    fpush(&c, -(double)NAN);
                         fxam(&c);
                         CHECK((c.fpu_sw & 0x4500) == 0x0100);   /* NaN class */
                         CHECK((c.fpu_sw & 0x0200) == 0x0200);   /* C1 = sign */
                         fdrop(&c);
    fpush(&c, -0.0);     fxam(&c); CHECK((c.fpu_sw & 0x0200) == 0x0200); fdrop(&c);
    ...
    x87_set_cw(&c, 0x037f);  CHECK(fround_cw(&c, 2.5) == 2.0);    /* nearest */
    x87_set_cw(&c, 0x077f);  CHECK(fround_cw(&c, 2.7) == 2.0);    /* down */
    x87_set_cw(&c, 0x0b7f);  CHECK(fround_cw(&c, 2.3) == 3.0);    /* up */
    fesetround(FE_UPWARD);
    x87_set_cw(&c, 0x0f7f);  CHECK(fround_cw(&c, 2.7) == 2.0);    /* truncate */
    x87_set_cw(&c, 0x037f);  CHECK(fround_cw(&c, 4503599627370497.0)
                                   == 4503599627370497.0);
    fesetround(FE_TONEAREST);
    /* FST to float honours RC explicitly too */
    x87_set_cw(&c, 0x077f);  CHECK(fto_float(&c, 1.0000001) == 1.0f);
    x87_set_cw(&c, 0x0b7f);  CHECK(fto_float(&c, 1.0000001) > 1.0f);
    x87_set_cw(&c, 0x037f);
    /* FUCOM is quiet for a quiet NaN; FCOM is not */
    c.fpu_sw = 0; fucom(&c, (double)NAN, 1.0);  CHECK((c.fpu_sw & 1u) == 0u);
    c.fpu_sw = 0; fcom(&c, (double)NAN, 1.0);   CHECK((c.fpu_sw & 1u) == 1u);
    { uint64_t sn = 0x7ff4000000000000ull; double snan;
      memcpy(&snan, &sn, 8);
      CHECK(is_snan(snan));
      CHECK(!is_snan((double)NAN));
      c.fpu_sw = 0; fucom(&c, snan, 1.0);      CHECK((c.fpu_sw & 1u) == 1u); }
    /* fset retags, fxch swaps tags with values */
    c.fpu_tag = 0xffff; c.fpu_top = 0;
    fpush(&c, 1.0); fpush(&c, 2.0);
    fset(&c, 0, 0.0);
    ...
    c.fpu_sw = 0; (void)fto_i32(&c, (double)INFINITY);
    ...
    c.fpu_sw = 0x0200u; c.fpu_top = 0;
    c.fpu_top = (c.fpu_top - 1u) & 7u; c.fpu_sw &= (uint16_t)~0x0200u;
...
{
    if (!harness_checks_run) (void)harness_header_selftest();
...
void harness_fnsave(const X86 *c, uint8_t *out)
{
    unsigned i;
    memset(out, 0, 108);
    out[0] = (uint8_t)c->fpu_cw;  out[1] = (uint8_t)(c->fpu_cw >> 8);
    uint16_t sw = fstsw(c);
    out[4] = (uint8_t)sw;         out[5] = (uint8_t)(sw >> 8);
    out[8] = (uint8_t)c->fpu_tag; out[9] = (uint8_t)(c->fpu_tag >> 8);
    for (i = 0; i < 8; i++) {
        double v = c->st[(c->fpu_top + i) & 7];
        uint64_t m;
        uint16_t se;
        if (isnan(v))      { m = 0xc000000000000000ull; se = 0x7fffu; }
        else if (isinf(v)) { m = 0x8000000000000000ull; se = v < 0 ? 0xffffu : 0x7fffu; }
        else if (v == 0.0) { m = 0; se = signbit(v) ? 0x8000u : 0u; }
        else {
...
double harness_st0(const X86 *c) { return c->st[c->fpu_top & 7]; }

/* Standalone translator harness has no scheduler or sampling thread. */
const int recomp_profile_enabled = 0;
void recomp_profile_push(uint32_t index) { (void)index; }
